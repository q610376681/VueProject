...
<a @click.prevent="getBook(book.id)" style="cursor:pointer" target="_blank">{{book.name}}</a>
</h5>
</div>

</div>
<div class="row" style="padding-left: 257px; margin-top: 65px;">
...
<!--分页-->
<nav aria-label="Page navigation" style="text-align: center; margin-top: 65px; " >
<ul class="pagination">
							<!--向左浏览页数  -->
							...
							<li v-if="pb.pageNum!==pb.start"><a
								aria-label="Previous"  @click.prevent="getClassifiedBooks(-1)"  style="cursor:pointer"> <span aria-hidden="true">&laquo;</span>
							</a></li>

							<!--显示每一页  -->
							<li v-for="i in pb.end" :key="i" v-if="i>=pb.start">
								<a href=""  @click.prevent="getClassifiedBooks(i-pageNum)">{{i}}</a>
							</li>
                ...
                <a href="" aria-label="Next" @click.prevent="getClassifiedBooks(1)">
...
export default {
  ...
  methods: {
    getClassifiedBooks (page) {
      this.pageNum += page
      this.$http
        .post(PSApi.getClassifiedBooks, {pageNum: this.pageNum, categoryId: this.categoryId})
        .then(result => {
          this.pb = result.body
          console.log(this.pb)
        })
